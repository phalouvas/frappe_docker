...
- Build worker image to include hrms with command in folder images/kainotomo `docker build --no-cache -f ./images/kainotomo/Containerfile . --tag phalouvas/erpnext-worker:14.60.1`
...
### Development Server
- Fetch from remotes
- Fix version 14.60.1 to new in repository gitops
...
- Export apps to variable and build image
  ...
  docker build  --no-cache --build-arg=FRAPPE_PATH=https://github.com/frappe/frappe \
    ...
    --tag=phalouvas/erpnext-worker:14.60.1 \
    --file=images/custom/Containerfile .

  docker push phalouvas/erpnext-worker:14.60.1
  docker tag phalouvas/erpnext-worker:14.60.1 phalouvas/erpnext-worker:version-14
...
- Export apps to variable and build image
  ...
  docker build  --no-cache --build-arg=FRAPPE_PATH=https://github.com/frappe/frappe \
    ...
    --build-arg=APPS_JSON_BASE64=$APPS_JSON_BASE64 \
    --tag=phalouvas/erpnext-worker:15.10.1 \
    --file=images/custom/Containerfile .

  docker push phalouvas/erpnext-worker:15.10.1
  docker tag phalouvas/erpnext-worker:15.10.1 phalouvas/erpnext-worker:version-15
...
- Activate github
  ```shell
    ...
    docker exec -it erpnext-v14-backend-1 bench --site all migrate
    docker compose --project-name erpnext-v15 down
