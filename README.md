...
- To create a new site with backend shell 
  ...
  bench new-site gpapachristodoulou.com --db-name gpapa --mariadb-root-password pRep5v3Nzw_aMMV --admin-password pRep5v3Nzw_aMMV --install-app erpnext --install-app payments
...
- Export apps to variable and build image
  ...
  docker build  --no-cache --build-arg=FRAPPE_PATH=https://github.com/frappe/frappe \
    ...
    --build-arg=APPS_JSON_BASE64=$APPS_JSON_BASE64 \
    --tag=phalouvas/erpnext-worker:15.22.0 \
    --file=images/azure/Containerfile .

  docker push phalouvas/erpnext-worker:15.22.0
  docker tag phalouvas/erpnext-worker:15.22.0 phalouvas/erpnext-worker:version-15
